...
export class Logger {
  log(message: string, context?: Record<string, any>) {
    ...
    console.log(
      JSON.stringify({ message, ...sanitized, timestamp: new Date() }),
    );
  }
...
**Key Principle**: _Assume breach_ - design every layer (Terraform, Kubernetes, application code) to contain damage if one layer is compromised. Defense in depth is essential for handling sensitive interview recordings.
